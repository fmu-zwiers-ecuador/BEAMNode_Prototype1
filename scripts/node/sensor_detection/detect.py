...
# NOTE: you had this line right after — it was re-overriding LOG_DIR.
# I'm keeping the first working version from get_log_dir().
...
# to write to the CWD: "detect_bme280.log"
# We already added a rotating file handler above, so we do NOT need this.
...
addr_table = {
    "TSL2591": 0x29,
    # ADDED: AHT family (AHT20 / AHT21 / AHTx) from Adafruit boards
    "AHT": 0x38,   # ADDED
    # if you later add more i2c sensors, just drop them in here
}
...
# =========================================================
# POST-DETECTION CONFIG UPDATES
# =========================================================

# FIXED: make this print nice whether it's a list or a string
if isinstance(devices, list):
    print("The following sensors are online:", ", ".join(devices) if devices else "None")
else:
    # devices is an error string
    print(devices)

# if TSL2591 was found, enable it in config
if isinstance(devices, list) and "TSL2591" in devices:
    set_config_flag(CONFIG_PATH, "tsl2591", "enabled", True)

# ADDED: if AHT was found, enable it in config
if isinstance(devices, list) and "AHT" in devices:  # ADDED
    set_config_flag(CONFIG_PATH, "aht", "enabled", True)  # ADDED
    set_config_flag(CONFIG_PATH, "aht", "i2c_bus", 1)     # ADDED
    set_config_flag(CONFIG_PATH, "aht", "address_hex", "0x38")  # ADDED
